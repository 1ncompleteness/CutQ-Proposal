...
\title{
  ...
  \large CSUN COMP 485 / Human-Computer Interaction.\\
  Final Project Report
}
\author{
  Jerry Ttuyan \quad and \quad Behrouz Barati B\\[0.3em]
...
barber is double-booked. CutQ centralizes booking, surfaces
...
FR-6  & Billing \& invoices   & Itemized charges, recurring packages, refunds.\\
...
FR-10 & Activity log          & Audit trail for bookings, edits, refunds, and role changes.\\
\bottomrule
\end{tabular}
...
\begin{description}[leftmargin=2.2em,style=nextline]
  \item[Accessibility (WCAG~2.1~AA)]\par
    \begin{itemize}[noitemsep]
      ...
      \item Honors Dynamic Type and OS-level reduce-motion.
    \end{itemize}
  \item[Performance]\par
    \begin{itemize}[noitemsep]
      \item Booking screens render in under one second on mid-tier
            devices.
      \item Server pagination on rosters and history, not all-at-once
            loads.
      \item Optimistic UI for booking confirmation, reconciled on
            error.
      \item Live Activity updates batched to preserve battery.
    \end{itemize}
  \item[Security and Privacy]\par
    \begin{itemize}[noitemsep]
      \item Stripe-style tokenization: card data never hits our
            database.
      \item Role-scoped APIs with server-enforced authorization.
      \item PII minimized, logs scrubbed of identifying detail.
...
Trust: \emph{"Did the booking really go through?"} &
  ...
  card. &
  Closes feedback loops that the user can see and re-check.\\
\addlinespace
...
\begin{table}[H]
\centering\small
\begin{tabular}{@{}rlp{3.4cm}p{8.5cm}@{}}
\toprule
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{figures/wireframes.png}
...
\vspace{0pt}
\centering
\includegraphics[width=0.65\linewidth]{figures/dashboard.png}
\end{minipage}\hfill
...
\vspace{0pt}
\centering
\includegraphics[width=0.65\linewidth]{figures/lock-screen.png}
\end{minipage}\hfill
...
Barber dashboard \emph{In Progress} pill confused with a system notification. &
  Repositioned to top-right with a calmer fill color.\\
\addlinespace
...
\begin{table}[H]
\centering\small
\begin{tabular}{@{}p{7.0cm}p{7.0cm}c@{}}
\toprule
